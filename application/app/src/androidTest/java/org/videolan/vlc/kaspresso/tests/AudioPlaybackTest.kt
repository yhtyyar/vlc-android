...
class AudioPlaybackTest : KaspressoUITest() {
    ...
    fun fullAudioPlaybackLifecycle() = run {
        ...
        step("ШАГ 2: Клик по треку, запушенному тестом") {
            ...
            // RecyclerViewActions.actionOnItem(hasDescendant(...), click()) стабильно не
            // находил элемент здесь (проверено дампом UI-дерева: структура и текст на месте,
            // "Found 0 items matching" тем не менее) — но хотя бы падал быстро и чисто.
            // onView(withText(...)).perform(click()) на этом экране ЗАВИСАЛ насмерть
            // (воспроизведено дважды — реальный hang, не медленный прогон). Кликаем по
            // content-desc самой строки (у неё он богатый и уникальный: "Audio track:
            // sample_audio.mp3, Duration: ...") — это clickable-родитель напрямую, без
            // hasDescendant-траверса и без сценария, вызвавшего зависание.
            onView(withContentDescription(startsWith("Audio track: sample_audio.mp3,"))).perform(click())
